...
def reduce(f, g, F, G):
	"""Use Babai's reduction to reduce (F, G) relatively to (f, g).

	(F, G) <-- (F, G) - k * (f, g), where k = round((F f* + G g*) / (f f* + g g*)).
	Corresponds to algorithm 12 (Reduce) of Falcon's documentation.
	"""
	n = len(f)
...
def check_ntru(f, g, F, G):
	...
	assert ((c[0] == q) and all(coef == 0 for coef in c[1:]))
	return
...
def ntru_solve(f, g):
	...
	Corresponds to algorithm 11 (NTRUSolve) of Falcon's documentation.
	"""
	n = len(f)
	if n == 1:
		f0 = f[0]
